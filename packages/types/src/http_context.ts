/**
 * Types pour le contexte HTTP d'AdonisJS avec les informations d'authentification
 * et d'autorisation spécifiques à Ona UI
 */

// Interface de base pour le contexte HTTP (simplifiée pour le package types)
...
export function hasPermission(ctx: BaseHttpContext, permission: string): ctx is PermissionHttpContext {
  const userPermissions = (ctx as any).userPermissions as string[] | undefined
