            ...
            <div className="absolute left-1/2 transform -translate-x-1/2">
              <div className="hidden md:flex items-center gap-8">
                {navigation.map((item) => {
                  ...
                  const baseClasses = "text-sm font-medium transition-colors flex items-center gap-2 px-3 py-2 rounded-lg"
                  const activeClasses = "text-slate-800 hover:text-slate-900 hover:bg-slate-100"
                  const disabledClasses = "text-slate-500 cursor-not-allowed"
                  
                  const content = (
                    <>
                      {item.name === 'Blocks' && <LucideLayoutPanelTop className="w-4 h-4" />}
                      {item.name === 'Templates' && <AppWindowMac className="w-4 h-4" />}
                      {item.name === 'Price' && <CreditCard className="w-4 h-4" />}
                      <span className={isDisabled ? "line-through" : ""}>{item.name}</span>
                      {item.badge && (
                        <span className="ml-1 px-2 py-0.5 text-xs font-medium bg-gradient-to-r from-orange-100 to-amber-100 text-orange-700 rounded-full ring-1 ring-orange-200/50">
                          {item.badge}
                        </span>
                      )}
                    </>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="hidden sm:flex items-center gap-1">
                <a
                  ...
                  className="p-2 text-slate-600 hover:text-[#C96342] transition-colors rounded-lg hover:bg-slate-100"
                >
